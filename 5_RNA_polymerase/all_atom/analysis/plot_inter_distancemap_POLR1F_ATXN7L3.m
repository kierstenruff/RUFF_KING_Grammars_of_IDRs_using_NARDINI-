...
constructs={'POLR1F_IDR1_w_ATXN7L3_IDR2'};
mol1={'POLR1F_IDR1'};
mol2={'ATXN7L3_IDR2'}
mytitles={'POLR1F IDR1 - ATXN7L3 IDR2'};
...
types{1}={'K'};
types{2}={'DE'};
mycolor3=[[55 71 157]/255; [225 57 55]/255];
...
for c=1:length(constructs)
    ...
    for r=1:length(frcreps)
        da=load(['../' constructs{c} '/FRC_test/epsval_0.0001_epsexp_6/' num2str(frcreps(r)) '/ana/inter_distanceMap_full.csv']); 
    ...
    for i=1:length(myseq1)-bsz+1
        ncpr(i)=(length(find(chargepos1(i:i+bsz-1)==1))-length(find(chargepos1(i:i+bsz-1)==2)))/bsz;
    end
    ...
    hold on
    area(1:1:length(myseq1),yn,'FaceColor',mycolor3(2,:))
    ...
    for i=1:length(myseq2)-bsz+1
        ncpr(i)=(length(find(chargepos2(i:i+bsz-1)==1))-length(find(chargepos2(i:i+bsz-1)==2)))/bsz;
    end
    ...
    yn = (mncpr - abs(mncpr))/2;
    % Plot the data using area function
    area(1:1:length(myseq2),yp,'FaceColor',mycolor3(1,:))
    hold on
    area(1:1:length(myseq2),yn,'FaceColor',mycolor3(2,:))
...
%print -painters -depsc 'two_molecule_POLR1F_IDR1_w_ATXN7L3_IDR2_scaled_distance_map_K_vs_DE.eps'
%print -painters -depsc 'two_molecule_POLR1F_IDR1_w_ATXN7L3_IDR2_scaled_distance_map_K_vs_DE_w_colorbar.eps'
